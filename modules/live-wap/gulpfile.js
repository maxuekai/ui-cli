const gulp = require('gulp'),
      ...
      px2rem = require('postcss-pxtorem'),//px转化为rem
...
const spriteHooks = function() {
	return {
		onUpdateRule: function(rule, token, image) {
			//百分比
      var backgroundSizes = (image.spriteWidth / 100);
...
gulp.task('css', function () { 
    var processors = [ 
                        ...
                        autoprefixer({
                          browsers: ['> 1%','last 2 version','Android >= 4.0','ios >= 8']
                        }),
                        px2rem({rootValue: 100, propList: ['*']}),
                        mixins,
                        precss
                     ]; 
...
gulp.task('img', function () {
  return gulp.src(['src/_img/*.{jpg,png,jpeg}','!src/_img/sprite/**/*.{jpg,png,jpeg}'])
         .pipe(sourcemaps.init())
